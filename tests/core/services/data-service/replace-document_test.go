...
func TestReplaceDocument(t *testing.T) {
	...
	// Send the add symbol request
	_, replaceDocumentResponseError := dataMediator.ReplaceDocument("testData",
		addDocumentResponse.ID, base.DummyMessage{Text: "otherData"})

	if replaceDocumentResponseError != nil {
		zap.L().Error("failed to replace document", zap.Error(replaceDocumentResponseError))
		t.Fatal()
	}

	var testMessage base.DummyMessage

	retrieveDocumentResponseError := dataMediator.RetrieveDocument(addDocumentResponse.Collection, addDocumentResponse.ID, &testMessage)

	if retrieveDocumentResponseError != nil {
		zap.L().Error("failed to retrieve document", zap.Error(retrieveDocumentResponseError))
		t.Fatal()
	}

	if testMessage.Text != "otherData" {
		zap.L().Error("retrieved document did not change", zap.String("text", testMessage.Text))
		t.Fatal()
	}
