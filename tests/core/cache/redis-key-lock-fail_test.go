...
func TestRedisKeyLockFail(t *testing.T) {
	...
	lockAcquired2, lockError := redisClient.RequestKeyLock("test", 1*time.Minute)
	...
	if lockAcquired2 == true {
		zap.L().Error("retrieved lock that we should not have")
		t.Fatal()
	}
